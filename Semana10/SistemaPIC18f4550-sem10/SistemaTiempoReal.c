...
void addbuff(char rcv) {//vector para [[numero de proceso]
   ...
   break;
    ...
    case 6:{
   //recibir velocidad deseada
  pw1=(int32)(var5+(var4)*10+(var3)*100);//
   if(pw1>500){
   pw1=500;}
   comando='V';
   break;
...
void pwem(int32 pw1,int32 T){
   ...
   pw2=T-pw1;
      ...
      ///////////////////////////////////////////////////////////////////////
      for(pw=1;pw<pw1;pw++){for(pw=1;pw<pw1;pw++)
      {
      for(pw=1;pw<20;pw++){for(pw=1;pw<20;pw++){}}
      output_high(pin_B2);
      }//lado de alta
      }
            
      for(pw=1;pw<pw2;pw++){for(pw=1;pw<pw2;pw++)
      {
      for(pw=1;pw<20;pw++){for(pw=1;pw<20;pw++){}}
      output_low(pin_B2);
      }//lado de baja
...
int parpadeo4=0;
   while(TRUE){
      ...
      for (k4=0;k4<20;k4++){ for(l4=0;l4<20;l4++){}}
      pwem(pw1,T);
      if(parpadeo4>20){
      printf("Listo4*");
      parpadeo4=0;
...
void prender_D5(){
  int parpadeo5=0;
   ...
   {    
      ...
      for (k5=0;k5<50;k5++){ for(l5=0;l5<50;l5++){}}    
...
int parpadeo6=0;
   while(TRUE){
      ...
      for (k6=0;k6<150;k6++){ for(l6=0;l6<50;l6++){}}
      if(parpadeo6>20){
      printf("Listo6*");
      parpadeo6=0;
      }
...
{parpadeo7=0;
   ...
   {  parpadeo7++;      
      ...
      if(rpm_deseada==0){
  //  output_low(pin_D4);
      output_low(Pin_B1);//deactiva el enable;
      }
      else{
      output_high(Pin_B1);
      if(rpm_deseada !=0){
...
int parpadeo8=0;
   while(TRUE){
      output_toggle(Pin_D2);
      for (k8=0;k8<150;k8++){ for(l8=0;l8<50;l8++){}}
      if(parpadeo8>20){
      printf("Listo8*");
      parpadeo8=0;
      }
